...
| 🟠 2.44 | **NAGRYWARKA wydzielona do osobnej aplikacji — w SmartGCS** (nie tu): worktree `C:\Soft\gcs-DRON\RPI-nagrywarka`, gałąź `feat/nagrywarka-osobno`, katalogi `nagrywarka/` + `wspolne/`. ⛔ Moje pierwsze założenie „kod tylko na malinie" było błędne — pulpit od 30.08 jest w SmartGCS (`RPI`), malina to wdrożenie. Testy, kontrola nazw i CLI przeszły na laptopie; **wgranie na malinę i restart `gcs-pulpit` — niesprawdzone** | dok/NAGRYWARKA_WYDZIELENIE.md |
