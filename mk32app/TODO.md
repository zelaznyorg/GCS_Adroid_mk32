...
| ✅ 2.30 | **ROZSTRZYGNIĘTE 2026-09-05 15:43 — obraz z kamery Mavica w Pilocie 2 JEST widoczny w zrzucie ekranu** (zrzut ekranu stacji: kafelek „DJI — ekran aparatury” pokazuje podgląd z kamery, HUD Pilota, mapę; nic nie jest czarne). Ten kontroler nie ustawia `FLAG_SECURE`, droga zrzutu ekranu działa dla Mavic 3. Do spisania: model kontrolera i wersja Pilota 2. Pierwotnie: ⛔ Sprawdzić, czy DJI nie blokuje zrzutu (`FLAG_SECURE`) — jeśli tak, obraz będzie czarny i nie da się tego obejść. Stacja zgłasza podejrzenie po przepływności; rozstrzyga spojrzenie na podgląd | tamże |
...
| ✅ 2.50 | **Panel ADMIN przebudowany na karty (2026-09-04, uwaga Toma: „nieintuicyjny, w polu nie ma czasu na zgadywanie, gdzie co jest”).** Było: jedna karta, osiem sekcji jedna pod drugą. Jest: ZAPROSZENIA · DOSTĘP · NOWE ŹRÓDŁO · ŹRÓDŁA · ARCHIWUM · DIAGNOSTYKA — jedna karta = jedno pytanie, opis pod paskiem kart, kroki 1-2-3 w kartach-czynnościach (zaproszenie, nowe źródło), na końcu dodawania drona od razu adres RTMP i hasło do przepisania. Kod: `web/src/Admin.jsx` (szkielet) + `web/src/admin/*.jsx`; każda karta odświeża własne dane tylko, gdy otwarta. Ostatnia karta pamiętana (`dron15.admin.karta`). Wgrane na GSB 2026-09-04 wieczorem. ⚠ Do sprawdzenia na stacji pokrętłem: kolejność ogniska po pasku kart | dok/DOSTEP_I_UZYTKOWNICY.md §4 |
| ✅ 2.51 | **Pierwszy obraz z kontrolera DJI na stacji — 2026-09-05 14:12** (Horyzont, 1440×800, 15 kl./s, przez Wi‑Fi routera Plusa `192.168.100.98` → hEX → `.88.30`). Trzy usterki znalezione i naprawione tego samego dnia: (a) złe hasło → aplikacja mówiła „stacja nie odpowiada” (stacja zamykała gniazdo bez słowa; instrukcja w DJI.md kazała wpisać hasło urządzenia zamiast hasła źródła); (b) **nieruchomy ekran = 0 bajtów z kodera** (MediaCodec z powierzchni), MediaMTX po 10 s `i/o timeout` zrzucał RTMP, ffmpeg padał, a gniazdo aparatury wisiało jako nadawca — kolejne połączenia „odrzucony drugi nadawca” ×20; (c) okno Panoramy uruchomione ręcznie po ssh trzymało profil Chromium — kafelek „się zamykał”. Naprawy: `zrzut.mjs` (przejęcie po 2 s ciszy, koniec ffmpeg = rozłącz, strażnik 60 s, odpowiedź `zle-haslo`/`zajete`), MediaMTX `readTimeout` 60 s, Horyzont `KEY_REPEAT_PREVIOUS_FRAME_AFTER`, `podglad.sh` ubija starą instancję, blok DANE APARATURY w ŹRÓDŁA. ⚠ Otwarte: Horyzont ma czytać odpowiedź stacji i przestać ponawiać przy złym haśle; SPRAWDŹ ŁĄCZE ma sprawdzać hasło; test FLAG_SECURE (2.30) z Pilotem 2 na wierzchu wciąż nie wykonany | dok/DJI.md, dok/PROBY_20260904.md B1 |
| ✅ 2.52 | **Tor analogowy CVBS w Panoramie (2026-09-05 15:30).** Źródło pobierane `cvbs` = `rtsp://127.0.0.1:8554/uav` — czysty H.264 640×480/30 z usługi `pi5-uas-rtsp` (PI5setup, nie ruszamy), tej samej, z której korzysta NAGRYWARKA i ATAK. MediaMTX ciągnie na żądanie (`started on demand` gdy ktoś patrzy), więc bez widza analog nie kosztuje nic. Dodane przez API admina, widoczne; mozaika ma teraz 4 kafelki (2×2). ⚠ Do 15:50 kafelek pokazywał „PRZEGLĄDARKA NIE ODTWARZA TEGO KODEKA”, a bez okna KAMERA CVBS + HUD klatki nie płynęły w ogóle (0 pakietów w 30 s) — kamera i publikacja żyły w oknie. Tom rozdzielił to na usługę bezgłową `hdmi-cvbs-capture` + lekki HUD (2026-09-05 ~15:55). Zmierzone po zmianie: H.264 **Constrained Baseline, yuv420p, 640×480, 30 kl./s, klatka kluczowa co 1 s, bez B-klatek** — i kafelek CVBS gra NA ŻYWO w mozaice obok ZR30 i DJI (zrzut ekranu stacji 15:59). Strona główna odświeża listę źródeł sama co 20 s — wcześniej nowe źródło pojawiało się dopiero po przeładowaniu | dok/REJESTRATORY.md, serwer/README.md |
