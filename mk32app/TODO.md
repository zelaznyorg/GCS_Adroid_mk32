...
| 🔴 2.42 | ⛔ **DWA REJESTRATORY IP NA JEDNEJ MASZYNIE — rozstrzygnąć podział ról.** Pulpit ma wbudowany kafelek **⏺ NAGRYWARKA** (`gcs_pulpit/nagrywarka.py`, wieloźródłowa, lista edytowalna z ekranu, ffmpeg `-c copy` → `/var/lib/gcs/nagrania/<id>/*.mkv`, REC na belce i na panelu GC9A01), a stacja ma **archiwum** (MediaMTX record → `/var/lib/dron15/archiwum`). **ZR30 jest w obu** i oba ciągną ją osobno przez łącze radiowe. DJI jest tylko u nas, CVBS tylko u nich. Propozycja: stacja = rozdzielnia strumieni (MediaMTX wystawia lokalnie RTSP na `127.0.0.1`, port ≠ 8554), NAGRYWARKA = jedyny rejestrator obrazu; u nas `wideo: nie`, zostaje `.tlog` | ta rozmowa |
