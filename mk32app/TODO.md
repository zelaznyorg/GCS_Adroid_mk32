...
| 🟠 2.40 | **Edycja źródeł w panelu ADMIN** — dodawanie, zmiana i usuwanie z przestawieniem MediaMTX na żywo. Maszyneria już jest (`server/mediamtx.mjs` = klient portu 9997 przeniesiony z nagrywarki, `writeZrodla` waliduje i zapisuje); brakuje trzech punktów API i formularza. ⏸ **Wstrzymane do czasu, aż stacja wstanie** — najpierw zobaczyć, co na niej jest | dok/DJI.md §8 |
| 🟠 2.41 | Sprawdzić na stacji **ścieżkę RTSP `pi5-uas-rtsp`** (port 8554, obraz CVBS z `PI5setup full`) i rozważyć wpięcie jej jako źródła obok ZR30 i DJI. ⚠ Dołoży nagrywanie u nas, nie zastąpi `pi5-camera-recorder` — ten pisze na własną kartę SD, której nie ruszamy | dok/WDROZENIE_RPI.md |
