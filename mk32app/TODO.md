...
| 🔴 2.44 | **Wydzielić NAGRYWARKĘ do osobnego katalogu w Smart GCS** — dziś kod żyje tylko na malinie (`~/gcs-src/pulpit/gcs_pulpit/nagryw*.py`, bez git), zszyty z pulpitem przez `okno.py`. Cel Toma: rozwijać niezależnie. Najpierw zdjąć źródła i opisać sprzężenia (belka REC, kafelki, protokół panelu GC9A01, `ZRODLA_STACJI`) | dok/REJESTRATORY.md §7 |
