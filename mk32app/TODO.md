...
| 🔴 2.44 | **Wydzielić NAGRYWARKĘ do osobnego katalogu w Smart GCS** — dziś kod żyje tylko na malinie (`~/gcs-src/pulpit/gcs_pulpit/nagryw*.py`, bez git), zszyty z pulpitem przez `okno.py`. Cel Toma: rozwijać niezależnie. Źródła zdjęte, sprzężenia opisane, plan trzech kroków gotowy — **czeka na decyzję Toma**: czy pulpit wchodzi do repo (migawka) i gdzie katalog | dok/NAGRYWARKA_WYDZIELENIE.md |
